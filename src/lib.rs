...
pub extern "C" fn generated_on(
    ...
    write!(dest_slice, "{}\x00", GeneratedOn { server, date }).unwrap();
    dest
}

struct GeneratedOn<'a> {
    server: &'a bindings::server,
    date: &'a str,
}
impl<'a> std::fmt::Display for GeneratedOn<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let pkgname = unsafe { CStr::from_ptr(self.server.pkgname).to_str().unwrap() };
        if self.server.want_server_id == 1 {
...
pub extern "C" fn default_reply_impl(
    ...
    let date = unsafe { CStr::from_ptr(date.as_ptr()).to_str().unwrap() };
