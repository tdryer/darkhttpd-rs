...
fn test_forward(args: &[&str], url: &str, host: &str, location: &str) {
    ...
    let response = server
        .send(Request::new(url).with_header("Host", host))
        .unwrap();
...
#[test]
fn forward_url_no_leading_slash() {
    let server = Server::with_args(FORWARD_ARGS);
    let response = server
        .send(Request::new("foo").with_header("Host", "example.com"))
        .unwrap();
...
fn mimemap() {
    ...
    for (filename, content_type) in files {
        ...
        let response = server
            .send(Request::new(&format!("/{}", filename)))
            .unwrap();
        assert_eq!(response.status(), "200 OK");
...
fn dir_redirect() {
    ...
    let response = server.send(Request::new("/mydir")).unwrap();
    assert_eq!(response.status(), "301 Moved Permanently");
...
fn test_file_get(path: &str) {
    ...
    let response = server.send(Request::new(path)).unwrap();
    assert_eq!(response.status(), "200 OK");
...
fn file_head() {
    ...
    server.create_file("data.jpeg").write_all(&data).unwrap();
    let response = server
        .send(Request::new("/data.jpeg").with_method("HEAD"))
        .unwrap();
    assert_eq!(response.status(), "200 OK");
    assert_eq!(response.header("Accept-Ranges"), Some("bytes"));
    assert_eq!(
...
fn if_modified_since() {
    ...
    let response = server
        .send(Request::new("/data.jpeg").with_header("If-Modified-Since", last_mod))
        .unwrap();
...
fn test_range(range_in: String, range_out: (usize, usize), range_data: (usize, usize)) {
    ...
    server.create_file("data.jpeg").write_all(&data).unwrap();
    let response = server
        .send(Request::new("/data.jpeg").with_header("Range", &range_in))
        .unwrap();
...
macro_rules! test_bad_range {
    ($name:ident, $range:expr) => {
        ...
        fn $name() {
            ...
            server.create_file("data.jpeg").write_all(&data).unwrap();
            let response = server
                .send(Request::new("/data.jpeg").with_header("Range", &$range))
                .unwrap();
...
#[test_case("/../", "You requested an invalid URL.\n" ; "invalid up dir")]
#[test_case("/./dir/./../../", "You requested an invalid URL.\n" ; "fancy invalid up dir")]
fn is_invalid(url: &str, message: &str) {
    let server = Server::new();
    for (version, line_ending) in HTTP_CLIENT_CASES {
        let response = server
            .send(
...
fn multiple_send() {
    ...
    server.create_file("data.jpeg").write_all(&data).unwrap();
    let response = server
        .send_stream(&mut stream, Request::new("/data.jpeg"))
        .unwrap();
